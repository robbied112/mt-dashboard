...
Usage:
  python3 build_rep_dashboard.py [--base-dir /path/to/Raw Exports] [--template /path/to/template.html] [--output /path/to/index.html]
"""
import openpyxl
import json
import re
...
def main():
    ...
    base_dir = args.base_dir or script_dir
    ...
    if not template:
        candidates = [f for f in os.listdir(output_dir) if f.endswith('_MT_BI_RepDashboard.html')]
        if candidates:
            template = os.path.join(output_dir, sorted(candidates)[-1])
        else:
            parent = os.path.dirname(base_dir)
            candidates = [f for f in os.listdir(parent) if f.endswith('_MT_BI_RepDashboard.html')]
            if candidates:
    ...
    if not output:
        ...
        output = os.path.join(output_dir, f'{date_str}_MT_BI_RepDashboard.html')

    result = inject_data_into_html(template, js_data, output)

    # Also create/update index.html for GitHub Pages
    index_path = os.path.join(os.path.dirname(output), 'index_rep.html')
