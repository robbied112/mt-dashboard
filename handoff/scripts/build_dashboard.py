...
Usage:
  python3 build_dashboard.py [--base-dir /path/to/Raw Exports] [--template /path/to/template.html] [--output /path/to/index.html]
"""
import openpyxl
import json
import re
...
def main():
    ...
    parser.add_argument('--base-dir', default=None, help='Path to Raw Exports folder')
    ...
    parser.add_argument('--output-dir', default=None, help='Output directory (will create dated file + index.html)')
    args = parser.parse_args()

    # Determine paths
    script_dir = os.path.dirname(os.path.abspath(__file__))

    base_dir = args.base_dir or script_dir  # Default: same directory as script
    if not os.path.isdir(base_dir):
        base_dir = script_dir  # Fallback: look for files in same directory
    ...
    if not template:
        # Look for most recent *_MT_BI_Dashboard.html
        candidates = [f for f in os.listdir(output_dir) if f.endswith('_MT_BI_Dashboard.html')]
        if candidates:
            template = os.path.join(output_dir, sorted(candidates)[-1])
        else:
            # Check parent directory
            parent = os.path.dirname(base_dir)
            candidates = [f for f in os.listdir(parent) if f.endswith('_MT_BI_Dashboard.html')]
            if candidates:
                template = os.path.join(parent, sorted(candidates)[-1])
    ...
    if not output:
        ...
        output = os.path.join(output_dir, f'{date_str}_MT_BI_Dashboard.html')

    result = inject_data_into_html(template, js_data, output)

    # Also create/update index.html for GitHub Pages
    index_path = os.path.join(os.path.dirname(output), 'index.html')
    if index_path != output:
        import shutil
        shutil.copy2(output, index_path)
        print(f"Also copied to: {index_path}")
